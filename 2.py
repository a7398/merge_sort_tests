...
def add_student(conn, name):
    conn.execute('INSERT INTO s(name) VALUES(?)', (name,))

def add_course(conn, title):
    conn.execute('INSERT INTO c(title) VALUES(?)', (title,))

def add_student_course(conn, sid, cid):
    conn.execute('INSERT INTO sc(sid, cid) VALUES(?, ?)', (sid, cid))
